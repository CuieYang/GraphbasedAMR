...
class Node_emblayer(nn.Module):  # 继承nn.Module  yang 20190205
    def __init__(self,opt,in_features,out_features):
        ...
        self.K = opt.Node_K
        # self.aggw = nn.Linear(out_features, out_features)

        # self.fc = nn.Sequential(
        #     nn.Dropout(opt.dropout),
        #     nn.Linear(self.inputfeature, self.outfeature, bias=self.outfeature))
        # self.bc = nn.Sequential(
        #     nn.Dropout(opt.dropout),
        #     nn.Linear(self.inputfeature, self.outfeature, bias=self.outfeature))
        # self.edge = nn.Sequential(
        #     nn.Dropout(opt.dropout),
        #     nn.Linear(3*self.outfeature, self.outfeature, bias=self.outfeature))
    ...
    def forward(self, fw_input,bw_input, edge_embs, fw_adj, bw_adj,bw_end,fw_end,fw_edgeid,bw_edgeid,bw_edgeend,fw_edgeend,fw_edgedep,bw_edgedep):
        ...
        for k in range(self.K):
            ...
            bw_edgedepend = torch.zeros(len(bw_edgedep)).long()
            fw_edgedepend = torch.zeros(len(fw_edgedep)).long()
            for i in range(len(bw_edgedep)):   # three lens are different
                bw_edgedepend[i] = torch.sum(torch.gt(bw_edgedep[i, :], -1))
                fw_edgedepend[i] = torch.sum(torch.gt(fw_edgedep[i, :], -1))
            ...
            edge_train_num = torch.sum(torch.gt(fw_edgedepend, 0))

            if edge_train_num>0:
                edge_hidden = Variable(torch.zeros(edge_train_num, 3*self.outfeature))
                if self.cuda:
                ...
                for i in range(len(bw_edgedep)):
                    ...
                    if fw_edgedepend[i]>0:

                        indices = Variable(fw_edgedep[i, 0:fw_edgedepend[i]].cuda())
                        fadj_x = torch.index_select(fw_input, 0, indices)
                        fadj_x = torch.mean(fadj_x, dim=0)
                        edge_dep = torch.cat((edge_dep, fadj_x), -1)

                        indices = Variable(bw_edgedep[i, 0:bw_edgedepend[i]].cuda())
...
class Node_embedding(nn.Module):  # 继承nn.Module  yang 20190205
    def __init__(self, opt,embs):
        ...
        self.edge_embs = nn.Parameter(torch.randn(40, opt.txt_rnn_size))
    ...
    def forward(self, packed_input: PackedSequence,fw_adj,bw_adj,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep):
        ...
        bw_end = torch.zeros(len(bw_adj)).long()
        fw_end = torch.zeros(len(fw_adj)).long()
        bw_edgeend = torch.zeros(len(bw_adj)).long()
        fw_edgeend = torch.zeros(len(fw_adj)).long()

        for i in range(len(fw_adj)):
            bw_end[i] = torch.sum(torch.gt(bw_adj[i,:], -1))
            fw_end[i] = torch.sum(torch.gt(fw_adj[i, :], -1))
            bw_edgeend[i] = torch.sum(torch.gt(bw_edgeid[i,:], 0))
            fw_edgeend[i] = torch.sum(torch.gt(fw_edgeid[i, :], 0))
        edge_embs = self.edge_embs
        fw_output, bw_output = self.gc1(fw_input, bw_input, edge_embs, fw_adj, bw_adj,bw_end,fw_end,fw_edgeid,bw_edgeid,bw_edgeend,fw_edgeend,fw_edgedep,bw_edgedep)
        # fw_output, bw_output,edge_hidden = self.gc2(fw_output, bw_output,edge_hidden, fw_adj, bw_adj, bw_end, fw_end,fw_edgeid,bw_edgeid,bw_edgeend,fw_edgeend,fw_edgedep,bw_edgedep)


        # fw_output, bw_output = self.node_sence(fw_output, bw_output,edge_hidden, fw_adj, bw_adj, bw_end, fw_end, fw_edgeid, bw_edgeid,bw_edgeend, fw_edgeend)
