...
import numpy as np
from models.ConceptModel import *
from models.MultiPassRelModel import *

from modules.GumbelSoftMax import renormalize,sink_horn,gumbel_noise_sample
from modules.helper_module import doublepack
...
class AmrModel(nn.Module):
    ...
    def __init__(self,opt,embs,fw_adj,bw_adj):
...
#srl index is from original node to recategorized, combine with posterior of recategorized concepts
    ...
    def forward(self, input,rel=False):
        ...
        if len(input)==9 and not rel:
            #training concept identification, alignBatch here indicating possibility of copying (or aligned by rule)
            srcBatch,tgtBatch,alginBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep = input
            probBatch,src_enc = self.concept_decoder(srcBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep)
     ...
     #       assert not np.isnan(np.sum(rel_prob[0].data.data.cpu().numpy())),("inside srl\n",rel_prob[0].data.data)
        ...
        # concept identification evaluation
            srcBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep = input
            probBatch,src_enc= self.concept_decoder(srcBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep)
            return probBatch
