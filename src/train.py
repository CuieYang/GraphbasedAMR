...
def train_parser():
    ...
    parser.add_argument('-folder', default="rawdata", type=str ,
                        help="""the folder""")
    ...
    parser.add_argument('-txt_rnn_size', type=int, default=256)
    parser.add_argument('-rel_rnn_size', type=int, default=256,
                        help='Number of hidden units in the rel/root LSTM encoder/decoder')
    parser.add_argument('-amr_rnn_size', type=int, default=256)
    ...
    parser.add_argument('-Node_K', type=int,default=3,
                        help="number of node encoder cycle")
...
def eval(model,AmrDecoder,data,dicts,rel=False):
    ...
    for batchIdx in range(len(data)):
        ...
        order, srcBatch,tgtBatch,alginBatch,rel_batch,rel_index_batch,rel_roles_batch,gold_roots,sourceBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep = data[batchIdx]
        probBatch = model((srcBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep),rel=False)
...
def trainModel(model,AmrDecoder, trainData, validData, dicts, optim,best_f1 = 0 ,best_epoch = 0,always_print = False):
    ...
    def trainEpoch(epoch):
        ...
        for i in range(min(len(trainData),bs)):
            ...
            order,srcBatch,tgtBatch,alginBatch,rel_batch,rel_index_batch,rel_roles_batch,gold_roots,sourceBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep =trainData[i]

            probBatch,posteriors_likelihood_score,src_enc = model((srcBatch,tgtBatch,alginBatch,fw_adjs,bw_adjs,fw_edgeid,bw_edgeid,fw_edgedep,bw_edgedep),rel=False)
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        p_r_f1 = P_R_F1(
            *[a + b for (a, b) in zip(rel_scores[1].t_p_tp, concept_scores[1].t_p_tp)]) if opt.rel else P_R_F1(
            *concept_scores[0].t_p_tp)
        varres["P"].append(p_r_f1[0])
        varres["R"].append(p_r_f1[1])
        varres["F"].append(p_r_f1[2])
        ...
        if (f1 > best_f1):
            ...
            best_f1 = f1
            best_epoch = epoch
            torch.save(model, 'valid_best.pkl')
...
def embedding_from_dicts( opt,dicts):
    ...
    def initial_embedding():
        ...
        word_initialized = 0
        lemma_initialized = 0
        word_embedding = torch.nn.Embedding(dicts["word_dict"].size(),
                                      300,   #size of glove dimension
                              padding_idx=PAD)
        lemma_lut = torch.nn.Embedding(dicts["lemma_dict"].size(),
                                  opt.lemma_dim,
                                  padding_idx=PAD)
        with open(embed_path, 'r',encoding='UTF-8') as f:
            for line in f:
                ...
                if id2 != UNK and id2 < lemma_lut.num_embeddings :
  ...
  #  print (dicts["sensed_dict"])
    if opt.initialize_word:
        ...
        word_fix_lut =  torch.nn.Embedding(dicts["word_dict"].size(),
                                                  300,
                                          padding_idx=PAD)
        lemma_lut = torch.nn.Embedding(dicts["lemma_dict"].size(),
                                  opt.lemma_dim,
                                  padding_idx=PAD)
    dep_lut = torch.nn.Embedding(dicts["dep_dict"].size(),
                              opt.txt_rnn_size, padding_idx=PAD)
    high_lut = torch.nn.Embedding(dicts["high_dict"].size(),
                              opt.lemma_dim,
                              padding_idx=PAD)

    pos_lut = torch.nn.Embedding(dicts["pos_dict"].size(),
                                  opt.pos_dim,
                                  padding_idx=PAD)
    ner_lut = torch.nn.Embedding(dicts["ner_dict"].size(),
                              opt.ner_dim,
                              padding_idx=PAD)


    rel_lut =  torch.nn.Embedding(dicts["rel_dict"].size(),
                              1)  #not actually used, but handy to pass number of relations
    cat_lut = torch.nn.Embedding(dicts["category_dict"].size(),
                              opt.cat_dim,
                              padding_idx=PAD)


    aux_lut = torch.nn.Embedding(dicts["aux_dict"].size(),
                              1,
                              padding_idx=PAD)
...
def main():
    dicts = read_dicts()
    # print(dicts["dep_dict"])
    print('Building model...')
    AmrDecoder = AMRDecoder(opt,dicts)

    with_jamr = "_with_jamr" #if opt.jamr else "_without_jamr"
    suffix = ".pickle"+with_jamr+"_processed"
    adjsuffix = "txt_dependency_adj.edgeinfo_processed"
    trainFolderPath = opt.folder+"/training/"
    trainingFilesPath = folder_to_files_path(trainFolderPath,suffix)
    trainingAdjFilesPath = folder_to_files_path(trainFolderPath, adjsuffix)

    devFolderPath = opt.folder+"/dev/"
    devFilesPath = folder_to_files_path(devFolderPath,suffix)
    devAdjFilesPath = folder_to_files_path(devFolderPath, adjsuffix)

    testFolderPath = opt.folder+"/test/"
    testFilesPath = folder_to_files_path(testFolderPath,suffix)
    testAdjFilesPath = folder_to_files_path(testFolderPath, adjsuffix)

    print("111",len(dicts["dep_dict"]))
    training_data = DataIterator(trainingFilesPath, trainingAdjFilesPath,opt,dicts["rel_dict"],len(dicts["dep_dict"]))
    dev_data = DataIterator(devFilesPath, devAdjFilesPath, opt, dicts["rel_dict"],len(dicts["dep_dict"]), volatile=True)

    # order,srcBatch,tgtBatch,alginBatch,rel_batch,rel_index_batch,rel_roles_batch,gold_roots,sourceBatch,fw_adjs,bw_adjs = dev_data[0]
    #     # print(i, "=" , fw_adjs.size())
    #     # print(i, "=" ,len(bw_adjs))
    # print(fw_adjs)
    # dev_data = DataIterator(devFilesPath,devAdjFilesPath, opt,dicts["rel_dict"],volatile = True)
    ...
    if opt.train_from is None:
        ...
        AmrModel = models.AmrModel(opt, embs,fw_adj=None,bw_adj=None)
        print(AmrModel)
        parameters_to_train = []
