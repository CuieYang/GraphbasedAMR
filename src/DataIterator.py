    ...
    def read_dep_graph(self, depadj_path):
        ...
        with open(depadj_path, 'r') as f:
            ...
            for line in lines:
                ...
                # nfw_adjs = np.ones((lens, 2)) * -1
                # nbw_adjs = np.ones((lens, 2)) * -1
                # fw_edgeid = np.zeros((lens, 10))
                # bw_edgeid = np.zeros((lens, 10))
                # fw_edgedep = np.ones((self.edgelen, 10)) * -1
                # bw_edgedep = np.ones((self.edgelen, 10)) * -1
                #
                # fk = 0
                # bk = 0
                # fgk = 0
                ...
                # fed = 0
                # bed = 0
                # for i in range(lens):  #the i-th token
                #     for j in range(len(fw_dict[str(i)])):
                #         nfw_adjs[i][fk] = fw_dict[str(i)][j]
                #         fk = fk+1
                #     for j in range(len(bw_dict[str(i)])):
                #         nbw_adjs[i][bk] = bw_dict[str(i)][j]
                #         bk = bk+1
                #     if len(fw_edge[str(i)])>0:
